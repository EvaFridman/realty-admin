...
async function create(req, res, next) {
    try {
        const photo = await photosService.addPhoto(req.params.id, req.body);
...
async function update(req, res, next) {
    try {
        const photo = await photosService.updatePhoto(req.params.id, req.params.photoId, req.body);
...
async function remove(req, res, next) {
    try {
        await photosService.deletePhoto(req.params.id, req.params.photoId);
        res.status(204).send();
    } catch (err) {
        next(err);
    }
...
async function setCover(req, res, next) {
    try {
        const photo = await photosService.setCover(req.params.id, req.params.photoId);
        sendResponse(res, 200, photo, null, null);
    } catch (err) {
        next(err);
    }
